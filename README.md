...
| `.claude/skills/packging-os-maintainer/scripts/validate-packging-os.cmd`              | Windows 一键治理校验入口：调用 `python` 或 `python3`，需系统已装 Python |
| `.claude/skills/packging-os-maintainer/scripts/validate-packging-os.sh`               | Mac/Linux 一键治理校验入口：自动选择 `python3` 或 `python` |
| `.claude/skills/packging-os-maintainer/scripts/validate-packging-os.py`               | Python 版最小一致性检查：跨平台底层校验器 |
...
- Windows：运行 `.claude\skills\packging-os-maintainer\scripts\validate-packging-os.cmd`，它调用 Python（`python` 或 `python3`），需要系统已装 Python。
- Mac / Linux：运行 `sh .claude/skills/packging-os-maintainer/scripts/validate-packging-os.sh`，它会按顺序寻找 `python3`、`python`。
- 高级用法：如果你明确想指定运行时，也可以直接运行底层 `validate-packging-os.py`。
