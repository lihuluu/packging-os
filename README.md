...
- 想检查系统有没有漂：Windows 运行 `validate-packaging-os.cmd`，Mac/Linux 运行 `validate-packaging-os.sh`
...
Packaging OS 使用 **9 层工作流**，每层有明确的输入、输出和交付标准：
...
| 技能                             | 功能         | 典型场景                                         |
| -------------------------------- | ------------ | ------------------------------------------------ |
| `packaging-os`              | 系统总控     | 不确定从哪开始时                                 |
| `packaging-os-maintainer`   | 系统治理     | 修改 skill、路由、模板或治理文档后做巡检与补同步 |
...
| `.claude/skills/packaging-os-maintainer/SKILL.md`                                    | Claude Code 系统治理入口：检查 skill、路由、模板和文档同步       |
...
| `.claude/skills/packaging-os-maintainer/scripts/validate-packaging-os.cmd`              | Windows 一键治理校验入口：调用 `python` 或 `python3`，需系统已装 Python |
| `.claude/skills/packaging-os-maintainer/scripts/validate-packaging-os.sh`               | Mac/Linux 一键治理校验入口：自动选择 `python3` 或 `python` |
| `.claude/skills/packaging-os-maintainer/scripts/validate-packaging-os.py`               | Python 版最小一致性检查：跨平台底层校验器 |
| `.claude/skills/project-memory-manager/scripts/project_file_utils.py`                | 项目文档安全写入工具：同目录临时文件 + 原子替换 + 残留清理 |
...
- Windows：运行 `.claude\skills\packaging-os-maintainer\scripts\validate-packaging-os.cmd`，它调用 Python（`python` 或 `python3`），需要系统已装 Python。
- Mac / Linux：运行 `sh .claude/skills/packaging-os-maintainer/scripts/validate-packaging-os.sh`，它会按顺序寻找 `python3`、`python`。
- 高级用法：如果你明确想指定运行时，也可以直接运行底层 `validate-packaging-os.py`。
...
- `.cmd` 和 `.sh` 是入口脚本，负责选择合适运行时；`.py` 是底层校验器，覆盖同一组核心治理检查。
- Windows 默认走 `.cmd`，需系统已装 Python。
- Mac 如果没有 Python 3，可安装 Python 3.
- 日常治理顺序建议固定为：先跑 `validate-packaging-os`，如果出现 `[temp-file]` 或 `[temp-dir]`，先用项目清理脚本 dry-run 看清单，再执行删除。
- 后续凡是脚本批量写入项目级 Markdown，优先复用 `project_file_utils.py` 的安全写入流程，避免半写入文件长期留在项目目录里。
...
_Packaging OS — 秩序不是终点，是让创造力得以生长的土壤_
