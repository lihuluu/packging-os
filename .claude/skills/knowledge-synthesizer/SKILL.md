...
3. 需要批量整理时，先参考 [batch-scan-rules.md](./references/batch-scan-rules.md)，并优先使用 [`./scripts/get-knowledge-coverage.ps1`](./scripts/get-knowledge-coverage.ps1) 盘点项目覆盖率。
...
8. V4 优先运行 [`./scripts/start-knowledge-review-session.ps1`](./scripts/start-knowledge-review-session.ps1)，自动准备 `Workspace/Knowledge/Operations/` 下的 `coverage/`、`queue/`、`current/` 和 `history/` 所需文件。
9. 如果不走一键启动，批量模式下再运行 [`./scripts/build-knowledge-review-packet.ps1`](./scripts/build-knowledge-review-packet.ps1)，生成 `current/current-review-session.md`。
...
处理工作区级文件写入时，优先复用 [`./scripts/knowledge-file-utils.ps1`](./scripts/knowledge-file-utils.ps1)；需要清理历史遗留临时文件时，使用 [`./scripts/cleanup-temp-files.sh`](./scripts/cleanup-temp-files.sh)。
