...
function Write-KnowledgeFileAtomic {
    param(
    ...
    $tempPath = Get-KnowledgeTempPath -Path $Path
    ...
    try {
        ...
        Remove-KnowledgeTempFiles -TargetPath $Path
    } catch {
        Write-Error ("Failed to write knowledge file: {0}" -f $Path)
