...
- 如果通过脚本批量写入或更新项目文档，应使用 `project-file-utils.ps1` 的原子写入流程；手工/Agent 写入后，日常治理必须通过 `validate-packging-os` 检查是否残留 `.tmp.*`。
...
- 批量检查项目卡是否落后于最新产出（PowerShell 底层脚本）：[check-memory-drift.ps1](./scripts/check-memory-drift.ps1)
...
- 项目文档安全写入工具：[project-file-utils.ps1](./scripts/project-file-utils.ps1)
- 清理项目目录下的临时文件（Windows，默认 dry-run）：[cleanup-project-temp-files.cmd](./scripts/cleanup-project-temp-files.cmd)
- 清理项目目录下的临时文件（Windows 底层脚本）：[cleanup-project-temp-files.ps1](./scripts/cleanup-project-temp-files.ps1)
